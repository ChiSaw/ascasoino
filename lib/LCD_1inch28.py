...
class LCD_1inch28(config.RaspberryPi):
    ...
    def ShowImage_WindowsOld(self, Xstart, Ystart, Xend, Yend, Image):
    ...
    def ShowImage_Windows(self, Xstart, Ystart, Xend, Yend, Image):
        ...
        # Utilize numpy broadcasting to perform operations on the whole array at once, which should be faster
        pix = self.np.zeros((self.width, self.height, 2), dtype=self.np.uint8)
        pix[..., [0]] = self.np.add(img[..., [0]] & 0xF8, img[..., [1]] >> 5)
        pix[..., [1]] = self.np.add((img[..., [1]] << 3) & 0xE0, img[..., [2]] >> 3)
        pix = pix.flatten().tolist()
        ...
        step = 240 * 2
        for i in range(Ystart, Yend - 1):
            Addr = Xstart * 2 + i * step
            self.spi_writebyte(pix[Addr : Addr + (Xend - Xstart) * 2])

    def ShowImageOld(self, Image):
        """Set buffer to value of Python Imaging Library image."""
        """Write display buffer to physical display"""
        imwidth, imheight = Image.size
        if imwidth != self.width or imheight != self.height:
    ...
    def ShowImage(self, Image):
        ...
        # Flattening array and converting to bytes to avoid conversion to list
        pix = pix.tobytes()

        self.SetWindows(0, 0, self.width, self.height)
        self.digital_write(self.DC_PIN, self.GPIO.HIGH)

        # You might need to adjust the chunk size here
        chunk_size = 4096
        for i in range(0, len(pix), chunk_size):
            self.spi_writebyte(pix[i : i + chunk_size])
